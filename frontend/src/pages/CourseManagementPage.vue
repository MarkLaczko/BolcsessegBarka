          ...
          <div class="card darkTheme">
            ...
            >
              ...
              >
                <template #body="slotProp">
                    <div class="d-flex justify-content-center">
                        <img v-if="slotProp.data.image" :src="'data:image/jpeg;base64,' + slotProp.data.image" alt="Course Image" style="max-height: 100px; max-width: 100%;">
                    </div>
                </template>
              </Column>
  ...
  export default {
    ...
    methods: {
      ...
      async deleteCourse(userId) {
        try {
          await http.delete(`/courses/${userId}`, {
            headers: {
              Authorization: `Bearer ${this.token}`,
            },
          });
      ...
      async updateCourse(data) {
        try {
          await http.put(`/courses/${data.id}`, data, {
            headers: {
              Authorization: `Bearer ${this.token}`,
            },
          });
